 ...
 * @class perc lanes
 */
class LaneLineSensor : public pattern::Task<LaneLineSensor>,
                     public UdpSocketServer<sim_ground_truth::LaneLines>{
 public:
  template<typename... OptionArgs>
  LaneLineSensor(stoic::cm::NodeHandle& nh, OptionArgs&&... option_args) :
  ...
  LaneLineSensor(stoic::cm::NodeHandle& nh, const pattern::TaskOptions& task_options) : Task(nh, task_options) {
    nh_ptr_ = &nh;
  }
  ~LaneLineSensor() = default;
  void run() override;
 private:
  bool init(const YAML::Node& node);
  bool convert2LanesMsg(const sim_ground_truth::LaneLines&, sim_perception::PerceptionLanes&);
 private:
...
WORKFLOW_ADD_TASK(::stoic::app::bridge::LaneLineSensor)
